...
# /sc:sc:sc:estimate - Development Estimation
...
## Usage
```
/sc:sc:estimate [target] [--type time|effort|complexity] [--unit hours|days|weeks] [--breakdown]
```
...
```
/sc:sc:estimate "user authentication system" --type time --unit days --breakdown
...
```
/sc:sc:estimate "migrate monolith to microservices" --type complexity --breakdown
...
```
/sc:sc:estimate "optimize application performance" --type effort --unit hours
...
**Next Step**: After estimation, user decides on timeline. Use `/sc:sc:workflow` for planning or `/sc:sc:implement` for execution.
